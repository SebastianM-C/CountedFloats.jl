...
[![CI](https://github.com/SebastianM-C/CountedFloats.jl/actions/workflows/CI.yml/badge.svg)](https://github.com/SebastianM-C/CountedFloats.jl/actions/workflows/CI.yml)
...
Not registered yet: `pkg> add https://github.com/SebastianM-C/CountedFloats.jl`.
...
- [GFlops.jl](https://github.com/triscale-innov/GFlops.jl) — counts FLOPs by Cassette-instrumenting
  the code, so it sees concrete `Float64` code too but only through Cassette's overdub (no GPU
  kernels). CountedFloats trades that generality for a plain number
  type: nothing to instrument, and the counts are bucketed by category and element type.
